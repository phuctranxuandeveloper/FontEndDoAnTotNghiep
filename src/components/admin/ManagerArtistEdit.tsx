...
import { getSingleArtist, updateArtist } from "../../server/ArtistService";
...
export const ManagerArtistEdit = () => {
  ...
  const handleSave = () => {
    if (artist !== null) {
      artist.nameArtist = nameInput;
      artist.avatar = urlAvatarInput;

      toast.promise(updateArtist(artist), {
        loading: "Saving.....",
        success: (data) => `Successfully save ${data.nameArtist}`,
        error: (err) => `This just happened: ${err}`,
      });
    }
  };

  const fetchingArtist = () => {
    if (id) {
      getSingleArtist(id)
        .then((data) => {
          setArtist(data);
  ...
  useEffect(() => {
    if (artist) {
      setNameInput(artist.nameArtist);
      setUrlAvatarInput(artist.avatar);
    }
  }, [artist]);
  return (
    <>
